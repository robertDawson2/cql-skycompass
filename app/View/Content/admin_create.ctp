...
<?php $this->start('jquery-scripts'); ?>
	var parentTag = '<?php echo isset($parent['Content']['tag']) ? $parent['Content']['tag'] : ''; ?>';
	$("#ContentName").change(function() {
		if ($("#ContentTag").val() == '') {
			$("#ContentTag").val(parentTag + '/' + $("#ContentName").val().replace(/[ ]/g, "-").replace(/[^A-Za-z0-9-_]/g, "").toLowerCase());
		}
	});
...
<form method="post" action="/admin/content/create/<?php echo $parentId; ?>">
