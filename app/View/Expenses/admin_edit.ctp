...
<?php echo $this->Form->create('edit', array('type' => 'file')); ?>
  <?= $this->Form->hidden('BillItem.id'); ?>
  <?= $this->Form->hidden('BillItem.bill_id'); ?>
  <?= $this->Form->hidden('BillItem.vendor_id'); ?>
  <?= $this->Form->hidden('BillItem.quantity'); ?>
            ...
            <div class="row">
              <div class="col-md-6">
                  <div class='form-group'>
                      ...
                      <select id='customerList'  data-placeholder='Select a organization or job...' class="form-control select2 validation" data-required='required' name='data[BillItem][customer_id]' style="width: 100%;">
                   <option></option>
                   <?php foreach($customers as $p): ?>
                    <?php $selected = "";
                    if($p['id'] == $this->data['BillItem']['customer_id'])
                        $selected = "selected";
                    ?>
                  ...
                  <input name='data[BillItem][amount]' value="<?= $this->data['BillItem']['amount']; ?>" type="text" class="form-control pull-right validation" data-required='required' data-type='number' />
              ...
              <div class="form-group">
                ...
                <select name='data[BillItem][item_id]' id='serviceItemList' data-placeholder='Select a service item...' class="form-control select2 validation" data-required = "required" style="width: 100%;">
                   ...
                   <?php foreach($services as $p): ?>
                    <?php $selected = "";
                    if($p['id'] == $this->data['BillItem']['item_id'])
                        $selected = "selected";
                    ?>
                    <option class="<?=$p['class']; ?>" value="<?=$p['id'];?>" <?= $selected; ?>>
                        <?=$p['name']; ?>
                ...
                <select name='data[BillItem][class_id]'  id='classItemList' data-placeholder='Select a class...'  class="form-control select2 validation" data-required = "required"  style="width: 100%;">
                    ...
                    <?php $selected = "";
                    if($p['id'] == $this->data['BillItem']['class_id'])
                        $selected = "selected";
                    ?>
                    <option class="<?=$p['class']; ?>" value="<?=$p['id'];?>" <?= $selected; ?>>
                        <?=$p['name']; ?>
                ...
                <textarea  name='data[BillItem][description]'  class='form-control' rows='5' width='100%'><?= $this->data['BillItem']['description']; ?></textarea>
              </div>
            </div> 
          </div>
