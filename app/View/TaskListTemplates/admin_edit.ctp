<?php  $this->assign('title', 'Edit Task List Template'); ?>
<div class="row">
    <div class="col-sm-6">
        <div class="box box-primary" style="position: fixed; width: 40%;">
            <div class="box-header ui-sortable-handle" style="cursor: move;">
              ...
              <h3 class="box-title">Possible Task Items</h3>
...
<div class="box box-primary">
    <form id='templateForm' method='POST' action='/admin/TaskListTemplates/edit'>
        <input type='hidden' name='data[TaskListTemplate][id]' value='<?= $this->data['TaskListTemplate']['id']; ?>' />
            <div class="box-header ui-sortable-handle" style="cursor: move;">
              <i class="ion ion-clipboard"></i>

              <h3 class="box-title">Current Task List</h3>
            ...
            <div class="box-body">
                <div class="box-tools" style="padding-bottom: 10px; border-bottom: 2px solid #aeaeae;">
                  ...
                  <input value = '<?= $this->data['TaskListTemplate']['name']; ?>' name='data[TaskListTemplate][name]' type="text" class="input form-control" placeholder="Enter Full List Name Here...">
                    </div>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <label>Description</label>
                            <textarea name='data[TaskListTemplate][description]' class="input form-control"><?= $this->data['TaskListTemplate']['description']; ?></textarea>
                        </div>
                    </div>
              </div>
              <ul id='new-list' class="todo-list ui-sortable">
                <?php foreach($data['TaskItem'] as $item): ?>
...
<!--                  <input value="" type="checkbox">-->
                  <!-- todo text -->
                  <span class="text"><?= $item['long_name']; ?></span>
                  <!-- Emphasis label -->
                  <span style="cursor: pointer;" onclick="showMore(this);" class="show-details pull-right label label-primary">More details <i class="fa fa-angle-double-down"></i> </span>
                  <!-- General tools such as edit or delete-->
                  <div class="tools">
                    ...
                    <i class="fa fa-edit editable-item"></i>
                    <i class="fa fa-trash-o removeable-item" data-id='<?= $item['id']; ?>'></i>
                  </div>
                  <div class="row more-details" style="padding-top: 10px; padding-bottom: 10px; display: none;">
...
$("#saveTemplate").click(function() {
  ...
  $(".todo-list").todolist({
    ...
    $("#options-list .removeable-item").click(function(){
        $id = $(this).data('id');
    ...
    $("#new-list .removeable-item").click(function(){
        $id = $(this).data('id');
        if(confirm('Are you sure you want to remove this item?'))
        {
